...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item" aria-current="page"><a href="{{route("admin.products.index")}}">All Products</a></li>
...
<div class="card-body p-4">
    ...
    <form action="{{ route("admin.products.update",$product->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        @include("Admin.Dashboard.Products._form",['type' => "Update"])
            ...
            <tbody>
                @foreach($gallary as $img)
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ route("admin.products.feature.gallaryImage.update", $img->id) }}" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                        ...
                        <!-- Delete Form -->
                        <td>
                            <form action="{{ route("admin.products.feature.gallaryImage.destroy", $img->id) }}" method="POST">
                                @csrf
                                @method('delete')
